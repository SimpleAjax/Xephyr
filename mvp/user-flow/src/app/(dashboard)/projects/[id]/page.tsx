...
export default function ProjectPage({ params }: ProjectPageProps) {
  ...
  const allTasks = useAppStore((state) => state.tasks).filter(t => t.projectId === id);
  ...
  const projectTasks = allTasks.filter(t => t.hierarchyLevel === 1);
  const subtasks = allTasks.filter(t => t.hierarchyLevel === 2);
  ...
  const stats = {
    ...
    totalHours: allTasks.reduce((sum, t) => sum + (t.estimatedHours || 0), 0),
    unassigned: allTasks.filter(t => !t.assigneeId && t.status !== 'done').length,
  };
  ...
  const getAssigneeName = (assigneeId?: string) => {
    ...
    return mockData.getUserById(assigneeId)?.name;
  };
    ...
    return (
      <div key={task.id}>
        ...
        {hasSubtasks && isExpanded && (
          <div className="mt-1">
            {taskSubtasks.map(subtask => renderTaskRow(subtask, true))}
          </div>
        )}
      </div>
    );
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {projectTasks.length} main tasks, {subtasks.length} subtasks
                  </CardDescription>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                {mockData.getUsers()
                  .filter(u => allTasks.some(t => t.assigneeId === u.id))
                  .map(user => {
